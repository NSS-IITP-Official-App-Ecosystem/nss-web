  ...
  return (
    <>
      ...
      <section className="bg-olive-50 pt-10 pb-10 mb-8 mt-8">
        {/* Fixed: Replaced invalid max-w-300 with an arbitrary or standard utility max-w-[1200px] */}
        <div className="ps-2 pe-2 max-w-[1200px] ms-auto me-auto flex flex-col items-center" id="home-nss-about">
          {/* Fixed: Moved 'uppercase' from id into className */}
          <h3 id="home-nss-about-title" className="mb-5 text-center text-4xl font-bold text-primary uppercase">
            WE ARE NSS IIT PATNA!
          </h3>
          <p id="home-nss-about-detail" className="text-text text-sm max-w-[75ch] self-center">
            The National Service Scheme of IIT Patna is an initiative to bring about a change in the Social and Economic forms of the Society. NSS believes that "Service to Mankind is Service to GOD" and with this mindset the members of NSS are ready to carve out humanity out of every social cause. The Scheme runs with a motive to inculcate Social Responsibilties in the youth who will be tomorrow's work force of the Nation at Institute Level is headed by a panel of administrative body, supported by General Secretary and Mentors of Different Cells who give guidance to nearly 120 Enthusiastic and Energetic Volunteers.
          </p>
          {/* Fixed: Removed frameBorder="0" and replaced with the border-0 Tailwind class */}
          <motion.iframe 
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-100px" }}
            transition={{ duration: 0.8 }} 
            className="mt-5 rounded-4xl max-w-full border-0" 
            width="800" 
            height="458" 
      ...
      <section id="home-our-units">
        ...
        {/* Added flex-wrap to avoid container overflow on mobile viewports */}
        <div className="mt-5 mb-5 flex gap-5 justify-center flex-wrap">
          {
            units_data.items.map((item, i) => {
              return (
                <motion.div key={i} className="flex flex-col max-w-80 items-center rounded-2xl border-zinc-100 border-2 shadow-xl bg-orange-200">
                  {/* Fixed: Added required alt attribute for Next.js Image component optimization */}
                  <Image className="rounded-t-[inherit]" src={item.thumbnail} width={400} height={300} alt={item.title} />
                  <h2 className="text-2xl font-semibold uppercase mt-3 mb-3">{item.title}</h2>
                  {/* Fixed: Flattened interactive element. Styled the Link directly instead of nesting it in a button */}
                  <Link href={item.action.url} className="button-primary mt-3 mb-3 inline-block">
                    {item.action.text}
                  </Link>
                </motion.div>
              )
