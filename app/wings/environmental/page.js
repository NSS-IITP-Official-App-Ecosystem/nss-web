...
import EnvironmentalWingClient from "./EnvironmentalWingClient";

export const dynamic = 'force-dynamic';

export default async function EnvironmentalWingPage() {
    let dbEvents = [];
    try {
        ...
        if (!error && allEvents) {
            // Filter events belonging to 'environment' and session '2025-2026'
            const wingEvents = allEvents.filter(e => 
                e.event_wings?.some(ew => ew.wings?.slug === 'environment')
            );
        ...
        console.error("Failed to fetch environmental wing events from database:", err);
    }

    return <EnvironmentalWingClient events={dbEvents} />;
}