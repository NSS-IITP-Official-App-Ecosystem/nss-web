...
import PrernaWingClient from "./PrernaWingClient";

export const dynamic = 'force-dynamic';

export default async function PrernaWingPage() {
    let dbEvents = [];
    try {
        ...
        if (!error && allEvents) {
            // Filter events belonging to 'prerna' and session '2025-2026'
            const wingEvents = allEvents.filter(e => 
                e.event_wings?.some(ew => ew.wings?.slug === 'prerna')
            );
        ...
        console.error("Failed to fetch prerna wing events from database:", err);
    }

    return <PrernaWingClient events={dbEvents} />;
}