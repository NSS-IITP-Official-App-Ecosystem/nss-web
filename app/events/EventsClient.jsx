...
export default function EventsClient({ initialEvents }) {
  ...
  const swachhataEvents = useMemo(() => {
    return initialEvents.filter(e => e.tags && e.tags.some(t => t.toLowerCase() === "swachhata hi seva"));
  }, [initialEvents]);
      ...
      <section className="bg-gradient-to-br from-emerald-950 via-slate-950 to-slate-900 text-white py-20 border-t border-emerald-900/30">
        <div className="max-w-7xl mx-auto px-6 sm:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {swachhataEvents.length === 0 ? (
              <div className="col-span-full text-center py-12 text-slate-400 font-semibold bg-white/5 border border-white/10 rounded-2xl">
                No Swachhata Hi Seva events scheduled for this session yet.
              </div>
            ) : (
              swachhataEvents.map((event) => {
                ...
                return (
                  <div 
                    ...
                    className="group bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 hover:border-emerald-500/30 transition-all duration-300 flex flex-col justify-between cursor-pointer"
                  >
                    <div>
                      <div className="relative aspect-video rounded-xl overflow-hidden mb-5 bg-slate-800">
                        <img 
                          src={event.images[0] || "/home_slider/nss_home.jpg"} 
                          alt={event.title}
                        ...
                        <div className="absolute top-3 left-3 bg-emerald-500 text-slate-950 font-extrabold text-[9px] px-2 py-0.5 rounded-md uppercase tracking-wider">
                          {event.tag.replace(' Wing', '')}
                        </div>
                      </div>
                      <h4 className="text-base font-extrabold text-white mb-2 group-hover:text-emerald-400 transition-colors line-clamp-2">
                        {event.title}
                      </h4>
                    ...
                    <div className="pt-4 border-t border-white/5 flex items-center justify-between text-[10px] font-bold text-emerald-400">
                      <span>DATE</span>
                      <span className="bg-emerald-500/10 px-2 py-1 rounded-md text-emerald-400">{formatDate(event.date)}</span>
                    </div>
                  </div>
                );
              })
